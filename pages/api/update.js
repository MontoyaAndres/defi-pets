...
const getImage = async (pet) => {
  ...
  //check pet.health / pet.stage
  let image = egg;
  if (pet.id == 1) image = hatch;
  if (pet.id == 2) image = baby;
  if (pet.id == 3) image = adult;
  //if(pet.alive == false) image = dead;
  ...
  return '<svg width="480" height="480" enable-background="new 0 0 20 20" fill="#000000" version="1.1" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">' +
    ...
    '<text x="20.960583" y="121.00983" font-size="16.667px" style="line-height:1.25" xml:space="preserve"><tspan x="20.960583" y="121.00983" stroke-width=".41667">Name: ' + pet.word + '</tspan></text>' +
    '<text x="21.505835" y="142.38661" font-size="11.111px" style="line-height:1.25" xml:space="preserve"><tspan x="21.505835" y="142.38661" font-size="11.111px" stroke-width=".41667">Points: ' + pet.letters + '</tspan></text>' +
    '<text x="21.505835" y="163.41194" font-size="11.111px" style="line-height:1.25" xml:space="preserve"><tspan x="21.505835" y="163.41194" font-size="11.111px" stroke-width=".41667">Health: ' + pet.remaining + '</tspan></text>' +
    '</g>' +
    '</svg>'
}

const getPet = async (tokenId) => {
  const table = "defi_pets_421614_475"; // "defi_pets_..."
...
export default async function handler(req, res) {
  ...
  const pets = await getPet(tokenId);
  ...
  if (pets.length > 0) {
    ...
    const result = {
      ...
      attributes: [
        { trait_type: 'Creator', value: pet.creator },
        { trait_type: 'Health', value: pet.letters }
      ]
    };
