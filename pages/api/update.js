...
const updatePet = async (tokenId, points, health, evolutionStage) => {
  ...
  let contract = new Contract(defiPets.arbitrumSepolia, defiPets.abi, signer);

  const updateTx = await contract.updatePet(
    tokenId,
    points,
    health,
    evolutionStage
...
export default async function handler(req, res) {
  try {
    ...
    if (pet) {
      ...
      const defiActivity = await getDeFiPositions(
        "0xd6eeF6A4ceB9270776d6b388cFaBA62f5Bc3357f"
      );
      ...
      let updatedHealth =
        pet.health +
        defiActivity.deposits * 10 +
        defiActivity.stakes * 20 -
        defiActivity.locks * 15;
      let updatedPoints =
        pet.points +
        defiActivity.rewards * 5 +
        defiActivity.airdrops * 1 -
        defiActivity.loans * 5;
      ...
      if (
        pet.evolutionStage < evolutionCriteria.length &&
